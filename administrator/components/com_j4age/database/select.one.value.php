...
class js_JSDbSOV extends js_JSDatabaseAccess
{
	...
	function getBuid( &$buid_result )
    {
        ...
        /**
         * We prevent recurrent calls! 
         */
        if($js_getBuild == null)
        {
            $query = 'SELECT MIN(i.visit_id)'
            . ' FROM #__jstats_impressions i WHERE i.visit_id > 0'
            ;
            $this->db->setQuery( $query );
            $buid_result_x = (int)$this->db->loadResult(); //must be (int)
            if ($this->db->getErrorNum() > 0)
            {
            ...
            $js_getBuild = $buid_result_x;
        }
		$buid_result = $js_getBuild;
		return true;
	}
	...
	function selectTotalNumberVisitorsWithoutBots( &$TotalNumberVisitorsWithoutBots_result ) {
		...
		. '   LEFT JOIN #__jstats_clients c ON (c.client_id = v.client_id)'
		. ' WHERE'
		. '   c.client_type='._JS_DB_IPADD__TYPE_REGULAR_VISITOR
		;
		$this->db->setQuery( $query );
	 ...
	 * @return true on success
	 */
	function selectNumberOfVisitors( $client_type, $include_summarized, $buid, $timestamp_from, $timestamp_to, &$NumberOfVisitors_result ) {
		...
		. '   LEFT JOIN #__jstats_clients c ON (c.client_id = v.client_id)'
		. ' WHERE'
		. '   '.( ($client_type!=='') ? 'c.client_type='.$client_type : '1=1' )
		. '   '.( ($include_summarized==false) ? 'AND v.visit_id >= '.$buid : '')
		. '   AND '.$this->getConditionStringFromTimestamps($timestamp_from, $timestamp_to)
		;
		$this->db->setQuery( $query );
	 ...
	 * @return true on success
	 */
	function selectNumberOfUniqueVisitors( $visitors_type, $cacheKey, $buid, $timestamp_from, $timestamp_to, &$NumberOfUniqueVisitors_result ) {
        ...
        $result = js_Cache::getCachedResult($cacheKey,$query);

        if($result != null)
        {
            $NumberOfUniqueVisitors_result =  (int)$result;
            return true;
        }
		...
		js_Cache::storeInCache($cacheKey,$query, $timestamp_from, $timestamp_to, -1, null, $NumberOfUniqueVisitors_result);
        return true;
	}
