...
/**
 *   Joomla Stats list of changes. Oryginal file from joomla 1.5.6
 * 
 * line  50 was: defined('JPATH_BASE') or die();
 * line  50 is:  defined('_JS_STAND_ALONE') or die ('JoomlaStats Stand Alone version: Direct Access to this location is not allowed.');
 *
 * line 250 was: JError::setErrorHandling(E_ERROR, 'die'); //force error type to die
 * line 251 was: $error = JError::raiseError( 500, JTEXT::_('Unable to load Database Driver:') .$driver);
 * line 252 was: return $error;
 * line 250 is:  //JError::setErrorHandling(E_ERROR, 'die'); //force error type to die
 * line 251 is:  //$error = JError::raiseError( 500, JTEXT::_('Unable to load Database Driver:') .$driver);
 * line 252 is:  return 'Unable to load Database Driver:'.$driver;
 *
 * line 260 was: JError::setErrorHandling(E_ERROR, 'ignore'); //force error type to die
 * line 261 was: $error = JError::raiseError( 500, JTEXT::_('Unable to connect to the database:') .$error);
 * line 262 was: return $error;
 *
 * line 260 is: JError::setErrorHandling(E_ERROR, 'ignore'); //force error type to die
 * line 261 is: $error = JError::raiseError( 500, JTEXT::_('Unable to connect to the database:') .$error);
 * line 262 is: return 'Unable to connect to the database:'.$error;
 *
 *
 */
	...
	function &getInstance( $options	= array() )
	{
		...
		if (empty($instances[$signature]))
		{
			...
			if (file_exists($path)) {
				...
				return $error;//JS never return object here!!
			}
