...
class js_JSStatus
{
	...
	function activeMethodTypeToText( $activeMethodType ) {
		switch( $activeMethodType) {
			case 'mod':
				return JTEXT::_( 'module <strong>j4age Activation Module</strong> (mod_j4age_activate)' );
				break;

			case 'plg':
				return JTEXT::_( 'plugin <strong>j4age Activation Plugin</strong> (bot_j4age_activate)' );
				break;
	...
	function showAnonymousUserCurrentTime( $isInstallPage, &$infoMsg ) {
		...
		$infoMsg[] = array( 'name' => JTEXT::_( 'j4age current time' ), 'description' => $txt );
	}
	...
	function isJSActivCheckFrontPage( &$isJSActivCheckFrontPage, &$iMsg ) {
		...
		if ( $allow_url_fopen == false ) {
			...
			$iMsg = ''
			. JTEXT::_( 'j4age is unable to perform self test because Your PHP have option "allow_url_fopen" turned off.' )
			. '<br/>'
			. JTEXT::_( 'It does NOT mean that j4age is not working correctly, only that j4age is unable test itself.' )
			;
			return false;
		}
		...
		if( $front_site_serialized === false ) {
			$iMsg = ''
			. JTEXT::_( 'j4age is unable verify front site.' )
			. '<br/>'
			. JTEXT::_( 'It does NOT mean that j4age is not working correctly, only that j4age is unable test itself.' )
			;
		    return false;
	...
	function checkDatabaseAndPhpFilesVersions( &$warningMsg ) {
		...
		if ($JSVerResult == false)
			$warningMsg[] = array( 'name' => JTEXT::_( 'j4age PHP and Database version' ), 'description' => JTEXT::_( 'j4age PHP version differ from j4age Database version. Probably You have j4age database from different version than j4age PHP files.' ) );
	}
	...
	function checkJSActivation( &$warningMsg, &$recommendationMsg, &$infoMsg ) {
		...
		{//A: check if user acivated JS in joomla by using 'Stand Alone' Activation method
			...
			if ( $txt != '' ) {
				$recommendationMsg[] = array( 'name' => JTEXT::_( 'j4age Acivation' ), 'description' => $txt );
			}
		}
		...
		$activationWarningMsg = '';
		{
			...
			if( $isJSActivated == true ) {
				if ( $isJSActivCheckFrontPageRes == false) {
					$infoMsg[] = array( 'name' => JTEXT::_( 'j4age Acivation' ), 'description' => $isJSActivCheckFrontPageiMsg );
				} else {
					if( $isJSActivCheckFrontPage == false ) {
						$activationWarningMsg .= JTEXT::_( 'You activated j4age by method (or methods)' ) . ': ';
			...
			if( count( $isJSActivationTypeInstalled ) > 0 ) {
				...
				. '<br />'
				. JTEXT::_( 'To activate j4age you have to enable/publish' ) . ' ';
				...
				$activationWarningMsg .= JTEXT::_( 'You have already installed j4age engine (component com_j4age), but j4age needs to be activated to work correctly!' )
				.'<br /><br />'
				. JTEXT::_( 'Choose one of below methods to activate j4age' )
				. ':<br />';
				...
				. '<li style="list-style-type: none;"><b>A)</b> ' . JTEXT::_( 'Install <strong>j4age Activation Module</strong> (mod_jstats_activate) <strong>(recommended)</strong>' ) . '</li>'
				. '<li style="list-style-type: none;"><b>B)</b> ' . JTEXT::_( 'Install <strong>j4age Activation Plugin</strong> (bot_jstats_activate)' ) . '</li>'
		...
		$warningMsg[] = array( 'name' => JTEXT::_( 'j4age Activation' ), 'description' => $activationWarningMsg );
	...
	function checkDevelopmentSnapshot( &$recommendationMsg ) {
		...
		if ($JSVerResult == true) {
			...
			if ($pos !== false)
				$recommendationMsg[] = array( 'name' => JTEXT::_( 'j4age development snapshot' ), 'description' => JTEXT::_( 'This is j4age development snapshot. Please install release version.' ) );
		}
	}
        ...
        if ($isInstallPage == false) { //we can not check WHOIS option during installation - it takes too much time!
			if ( $JSConf->enable_whois == true ) {
				...
				if( $isWhoIsOptionWorking == false ) {
					...
					. JTEXT::sprintf( 'j4age works correctly without this feature but %s will be unavailable or limited', JTEXT::_( 'recognition visitor countries' ) )
					;
				...
				if ( $isWhoIsOptionWorking == false ) {
					$desc = JTEXT::sprintf( 'Because WHOIS option is not working, You should turn off "%s" option in j4age configuration', JTEXT::_( 'WhoIs Support' ) );
