...
foreach($imagesArray as $key => $value) {
	echo '<img src="'.JURI::base(true).'/components/com_phocadownload/assets/images/blue/'.$value.'.png" style="display:none" alt="" />';
}
				...
				if ($valueDoc->textonly == 1) {
					...
					if ($valueDoc->description != '' && $valueDoc->description != '<p>&#160;</p>' && $valueDoc->description != '<p>&nbsp;</p>' && $valueDoc->description != '<p></p>' && $valueDoc->description != '<br />') {
						...
						if ($this->tmpl['display_description'] == 1 || $this->tmpl['display_description'] == 4) {
							$descriptionOutputTop 		= '<div class="pd-c-desc">'.$valueDoc->description.'</div>';
							$classMiddle 				= 'class="pdfile"';
						}
						if ($this->tmpl['display_description'] == 2 || $this->tmpl['display_description'] == 5) {	
							$descriptionOutputBottom 	= '<div class="pd-c-desc">'.$valueDoc->description.'</div>';
							$classMiddle 				= '';
						}
						...
						if ( $this->tmpl['display_description'] == 6 ) {	
							$details .= '<div>'.JText::_('Description').':<br />'.$valueDoc->description.'</div>';
							$descriptionOutputTop 	= '<div class="pd-c-pdfile">'.$details .'</div>';
							$classMiddle 				= 'class="pdfile"';
						}
						
						if ( $this->tmpl['display_description'] == 7 ) {	
							$details .= '<div>'.JText::_('Description').':<br />'.$valueDoc->description.'</div>';
							$descriptionOutputBottom 	= '<div class="pd-c-pdfile">'.$details .'</div>';
							$classMiddle 				= '';
						}
					...
					// OUTPUT
					echo '<div class="pd-c-download-item"><div class="pd-c-download-item2">';
					
					
					echo $descriptionOutputTop;			
					echo '<div '.$classMiddle.'>'.$displayNameHead. $imageFileNameThumbnail.'<div class="pd-document'.$this->tmpl['file_icon_size'].'" '.$imageFileName.'><div class="pd-float">';
					echo $linkDownloadB;
					echo $displayName;
					echo $linkDownloadE;
					if ($fileSize !='') {
					...
					if ($this->tmpl['display_play'] == 1) {
						if (isset($valueDoc->filename_play) && $valueDoc->filename_play != '') {
							...
							$canPlay	= PhocaDownloadHelper::canPlay($valueDoc->filename_play);
							if ($canPlay) {
					...
					if ($downloadOutput != '') {
						$buttonOutput .= '<div '.$classMiddle.'>'.$downloadOutput.'</div>';
						$buttonColumns--;
					}
					if ($detailOutput != '') {
						$buttonOutput .= '<div '.$classMiddle.'>'.$detailOutput.'</div>';
						$buttonColumns--;
					}
					if ($previewOutput != '') {
						$buttonOutput .= '<div '.$classMiddle.'>'.$previewOutput.'</div>';
						$buttonColumns--;
					}
					if ($playerOutput != '') {
						$buttonOutput .= '<div '.$classMiddle.'>'.$playerOutput.'</div>';
						$buttonColumns--;
					}
					
					for($i = 0;$i < $buttonColumns;$i++) {
						$buttonOutput = '<div '.$classMiddle.'> </div>'.$buttonOutput; 
					}
					// - - - - - 
					
					echo $descriptionOutputBottom;
					
					echo '<div class="pd-c-buttons">'.$buttonOutput.'</div>';
					echo '<div style="clear:both"></div>';
					echo '</div>' . "\n";
