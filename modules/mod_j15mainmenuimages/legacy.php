...
*/
function mosGetMenuLinkMMI($mitem, $level = 0, & $params, $open = null)
{
	global $Itemid;
	$txt = '';
	...
	if (!$current_itemid) {
		...
		if ($current_itemid == $mitem->id) {
			...
			if ($params->get('activate_parent') && isset ($open) && in_array($mitem->id, $open)) {
				...
				if ($mitem->type == 'url' && ItemidContainedMMI($mitem->link, $current_itemid)) {
					$id = 'id="active_menu' . $params->get('class_sfx') . '"';
				} else {
					$id = '';
				}
	...
	switch ($mitem->browserNav)
	{
		...
		case 1 :
			...
			$txt1 = '<a href="' . $mitem->url . '" target="_blank" class="' . $menuclass . '" ' . $id . '>';
			$txt2 = '</a>';
			break;
		...
		case 2 :
			...
			$txt1 = "<a href=\"#\" onclick=\"javascript: window.open('" . $mitem->url . "', '', 'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=780,height=550'); return false\" class=\"$menuclass\" " . $id . ">";
			$txt2 = "</a>\n";
			break;
		...
		case 3 :
			...
			$txt1 = '<span class="' . $menuclass . '" ' . $id . '></span>';
			break;

		default : // formerly case 2
			// open in parent window
			//$txt = '<a href="' . $mitem->url . '" class="' . $menuclass . '" ' . $id . '>' . $mitem->name . '</a>';
			$txt1 = '<a href="' . $mitem->url . '" class="' . $menuclass . '" ' . $id . '>';
			$txt2 = '</a>';
			break;
	}
...
*/
function mosShowVIMenuMMI(& $params)
{
	...
	mosRecurseVIMenuMMI(0, 0, $children, $open, $indents, $params);
}
...
* hierarchial menu
*/
function mosRecurseVIMenuMMI($id, $level, & $children, & $open, & $indents, & $params)
{
	if (@ $children[$id]) {
		...
		foreach ($children[$id] as $row) {
			...
			echo mosGetMenuLinkMMI($row, $level, $params, $open);

			// show menu with menu expanded - submenus visible
			if (!$params->get('expand_menu')) {
				if (in_array($row->id, $open)) {
					mosRecurseVIMenuMMI($row->id, $level +1, $children, $open, $indents, $params);
				}
			} else {
				mosRecurseVIMenuMMI($row->id, $level +1, $children, $open, $indents, $params);
			}
...
*/
function mosShowHFMenuMMI(& $params, $style = 0)
{
	$menu = & JSite::getMenu();
	$user = & JFactory::getUser();
	...
	if(is_array($rows) && count($rows)) {
		foreach ($rows as $row)
		{
			if ($row->access <= $user->get('aid', 0)) {
				$links[] = mosGetMenuLinkMMI($row, 0, $params);
			}
		}
	}
...
*/
function ItemidContainedMMI($link, $Itemid)
{
